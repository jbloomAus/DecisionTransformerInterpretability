...
class DecisionTransformer(nn.Module):
    def __init__(
        ...
        if isinstance(self.env.observation_space, Dict):
            ...
            self.one_hot_obs = (
                self.env.observation_space["image"].shape[-1] == 20
            )
        else:
        ...
        nn.init.normal_(
            ...
            mean=0.0,
            std=1 / ((env.action_space.n + 1) * self.d_model),
        )
        nn.init.normal_(
        ...
        nn.init.normal_(
            self.transformer.pos_embed.W_pos, cfg.initializer_range
        )
    ...
    def to_tokens(self, states, actions, rtgs, timesteps):
        ...
        action_embeddings = (
            self.get_action_embeddings(actions)
    ...
    def get_logits(self, x, batch_size, seq_length):
        x = x.reshape(batch_size, seq_length, 3, self.d_model).permute(
            0, 2, 1, 3
        )
    ...
    def check_input_sizes(self, states, actions, targets):
        assert (
            states.shape[0] == actions.shape[0] == targets.shape[0]
        ), "batch sizes must be the same"
        assert (
            states.shape[1] == actions.shape[1] == targets.shape[1]
        ), "block sizes must be the same"

    def get_time_embeddings(self, timesteps):
        assert (
            timesteps.max() <= self.max_timestep
