...
def test_get_logits(transformer):
    ...
    state_preds, action_preds, reward_preds = transformer.get_logits(x, batch_size, seq_length)

    assert state_preds.shape == (batch_size, seq_length, np.prod(transformer.environment_config.observation_space['image'].shape))
    assert action_preds.shape == (batch_size, seq_length, transformer.environment_config.action_space.n)
...
def test_predict_states(transformer):
    ...
    assert result.shape == (3, 4,  np.prod(transformer.environment_config.observation_space['image'].shape))

def test_predict_actions(transformer):
    d_model = transformer.transformer_config.d_model
    x = torch.rand(3, 4, d_model)
    result = transformer.predict_actions(x)
    assert result.shape == (3, 4, transformer.environment_config.action_space.n)
...
def test_get_state_embedding(transformer):
    ...
    assert result.shape == (batch_size, block_size, transformer.transformer_config.d_model)
...
def test_get_action_embedding(transformer):
    batch_size = 2
    block_size = 3
    actions = torch.randint(0, transformer.environment_config.action_space.n, (batch_size, block_size)).unsqueeze(-1)
...
def test_to_tokens(decision_transformer):
    ...
    token_embeddings = decision_transformer.to_tokens(states, actions, rewards, timesteps)
